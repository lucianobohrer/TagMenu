...
public final class HorizontalListViewModel<T: Displayable>: ObservableObject {
    ...
    @Published private(set) var items: [SelectableListItem<T>]
    private(set) var selectedItemsSubject = PassthroughSubject<[SelectableListItem<T>], Never>()
    ...
    init(items: [SelectableListItem<T>], title: String = "", selection: SelectionType = .single) {
        self.items = items
        self.title = title
    ...
    func handleSelection(item: SelectableListItem<T>) {
